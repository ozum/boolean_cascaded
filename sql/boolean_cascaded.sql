...
\echo Use "CREATE EXTENSION public.boolean_cascaded" to load this file. \quit
...
CREATE TYPE public.boolean_cascaded;

CREATE OR REPLACE FUNCTION boolcasin (pg_catalog.cstring)
RETURNS public.boolean_cascaded AS 'int4in' LANGUAGE 'internal'
IMMUTABLE
RETURNS NULL ON NULL INPUT
SECURITY INVOKER
COST 1;

CREATE OR REPLACE FUNCTION boolcasout (public.boolean_cascaded)
...
RETURNS public.boolean_cascaded AS 'int4recv' LANGUAGE 'internal'
IMMUTABLE
RETURNS NULL ON NULL INPUT
SECURITY INVOKER
COST 1;

CREATE OR REPLACE FUNCTION boolcassend (public.boolean_cascaded)
...
CREATE OR REPLACE FUNCTION boolcas (
  boolean
)
...
COMMENT ON FUNCTION boolcas_eq(public.boolean_cascaded, public.boolean_cascaded)
...
COMMENT ON FUNCTION boolcas_eq(public.boolean_cascaded, integer)
IS 'implementation of = operator';
...
COMMENT ON FUNCTION boolcas_eq(integer, public.boolean_cascaded)
IS 'implementation of = operator';
...
COMMENT ON FUNCTION boolcas_ne(public.boolean_cascaded, integer)
IS 'implementation of <> operator';
...
COMMENT ON FUNCTION boolcas_eq(public.boolean_cascaded, boolean)
IS 'implementation of = operator';
...
COMMENT ON FUNCTION boolcas_eq(boolean, public.boolean_cascaded)
IS 'implementation of = operator';
...
CREATE CAST (public.boolean_cascaded AS integer) WITHOUT FUNCTION AS IMPLICIT;
CREATE CAST (public.boolean_cascaded AS BOOLEAN) WITH FUNCTION "boolean"(public.boolean_cascaded) AS IMPLICIT;

CREATE CAST (integer AS public.boolean_cascaded) WITHOUT FUNCTION AS IMPLICIT;
CREATE CAST (BOOLEAN AS public.boolean_cascaded) WITH FUNCTION boolcas(boolean) AS IMPLICIT;
...
CREATE OR REPLACE FUNCTION boolcas_fk_name (
  p_table text,
...
COMMENT ON FUNCTION boolcas_fk_name(p_table text, p_referenced_table text, p_camel_case boolean)
...
--RAISE EXCEPTION 'Parent Changed: %', v_parent_changed;
    ...
    IF v_new IS DISTINCT FROM v_result THEN
        ...
        IF p_status_col = 'isActive' THEN
        	NEW."isActive" := v_result;
		ELSIF p_status_col = 'is_active' THEN
        	NEW.is_active := v_result;
        ELSE
            --RAISE NOTICE '% ID: %, %, %', TG_TABLE_NAME, NEW.id, v_new, format('%I=>%s', p_status_col, v_result);
        	IF v_result IS NOT NULL THEN
                --NEW := NEW #= format('%I=>%s', p_status_col, v_result)::hstore;                           -- For strings. Requires hstore extension.
                --NEW := NEW #= hstore(p_status_col, v_result);                                             -- Preferred hstore method for numbers. Requires hstore extension. CREATE EXTENSION IF NOT EXISTS hstore; -- WITH SCHEMA;
                SELECT json_populate_record (NEW, json_object(ARRAY['p_status_col', v_result])) INTO NEW;   -- JSON method undocumented. If error occurs in future use hstore method above. Extension kullanmamak için tercih edildi.
            END IF;
        END IF;
    END IF;
